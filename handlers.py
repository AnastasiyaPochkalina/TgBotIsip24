...
keyboard_markup2 = types.InlineKeyboardMarkup(row_width=3)
...
Бот создан при поддержки ФИО'''
    ...
    row_btns = (types.InlineKeyboardButton(text, callback_data=data) for text, data in arrayShelf1)

    keyboard_markup2.row(*row_btns)
    keyboard_markup2.add(
...
async def all_message(message: types.Message):
    ...
    if message.text == "Какой твой куратор?":
            await message.answer(reply_markup=keyboard_markup2)
